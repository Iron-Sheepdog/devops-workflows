...
| `gcp_project_id`                 | _(empty)_          | GCP project the review authenticates against via Workload Identity Federation. Use `isd-ai-innovation` for internal OpEx/analyst repos. Falls back to the caller's `VERTEX_PROJECT` repository/organization variable, then to `iron-sheepdog-dev`, if omitted.   |
| `gcp_location`                   | _(empty)_          | GCP location for Vertex AI inference. `gemini-3.5-flash` requires the `global` endpoint — it isn't served on regional endpoints like `us-central1`. Falls back to the caller's `VERTEX_LOCATION` repository/organization variable, then to `global`, if omitted. |
